...
import {
  ...
  TrendingUp, CalendarDays, HelpCircle
} from 'lucide-react';
...
function VehicleHero({ active, reminder, roadTax, insurance, onEdit }) {
  const serviceTone = !reminder?.known ? 'neutral' : reminder.overdue ? 'bad' : reminder.soon ? 'warn' : 'ok';
  const ServiceIcon = serviceTone === 'bad' ? AlertTriangle : serviceTone === 'warn' ? Clock3 : serviceTone === 'neutral' ? HelpCircle : CircleCheck;
  return (
    <section className="gl-hero" style={{ backgroundImage: active.photo ? `linear-gradient(90deg, rgba(5,12,23,.96) 0%, rgba(5,12,23,.82) 46%, rgba(5,12,23,.36) 100%), url(${active.photo})` : undefined }}>
      <div className="gl-hero-content">
        ...
        <div className="gl-hero-stats">
          ...
          <div className={`gl-service-status ${serviceTone !== 'ok' ? serviceTone : ''}`}>
            <div className={`gl-status-icon ${serviceTone !== 'ok' ? serviceTone : ''}`}><ServiceIcon size={30} /></div>
