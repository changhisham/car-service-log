  ...
  return (
    <Modal title={copy.title} onClose={onCancel}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', color: COLORS.steel, fontSize: 15, lineHeight: 1.5 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onCancel} style={{
            ...
            background: 'transparent', color: COLORS.paper, fontFamily: FONT_BODY, fontWeight: 600, fontSize: 15, cursor: 'pointer'
          }}>Cancel</button>
          <button
            onClick={onConfirm}
            style={{
              flex: 1, padding: '12px 16px', borderRadius: 10, border: 'none',
              background: COLORS.rust, color: '#fff', fontFamily: FONT_BODY, fontWeight: 700, fontSize: 15, cursor: 'pointer'
            }}>Delete</button>
        </div>
      </div>
    </Modal>
