...
import { Plus, Pencil, Trash2, MapPin, ClipboardList } from 'lucide-react';
  ...
  return (
    <div style={{ margin: '0 18px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <span style={{ fontFamily: FONT_DISPLAY, fontSize: 17.5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Logbook</span>
        <button onClick={onAdd} style={{
          display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none',
          color: COLORS.blue, fontFamily: FONT_BODY, fontWeight: 700, fontSize: 14.5, cursor: 'pointer'
        }}>
      ...
      {records.length === 0 ? (
        <div style={{
          ...
          <div style={{
            ...
            <ClipboardList size={20} color={COLORS.steelDim} />
          </div>
            ...
            return (
              <div key={r.id} style={{ display: 'flex', gap: 12, marginBottom: 12, position: 'relative' }}>
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, zIndex: 1, color: COLORS.blue
                }}>
                  <Icon size={16} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <SwipeToDelete onDelete={() => onDeleteRecord(r.id)}>
                    <div className="csl-card" style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: FONT_BODY, fontWeight: 700, fontSize: 15.5, marginBottom: 2 }}>{meta.label}</div>
                        <div style={{ fontSize: 13.5, color: COLORS.steel, fontFamily: FONT_MONO }}>
                          {fmtDate(r.date)} · {fmtKm(r.odometer)} km
                        </div>
                        {r.workshopName && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13.5, color: COLORS.steelDim, marginTop: 3 }}>
                            <MapPin size={11} /> {r.workshopName}{r.workshopLocation ? ` · ${r.workshopLocation}` : ''}
                          </div>
                        )}
                        {r.notes && <div style={{ fontSize: 14, color: COLORS.steelDim, marginTop: 5 }}>{r.notes}</div>}
                      </div>
                      {photos.length > 0 && (
                        <div style={{ position: 'relative', flexShrink: 0 }}>
                          ...
                          {photos.length > 1 && (
                            <span style={{
                              position: 'absolute', bottom: -4, right: -4, background: COLORS.blue, color: COLORS.bg,
                              fontSize: 12, fontWeight: 700, borderRadius: 999, padding: '1px 5px', fontFamily: FONT_MONO
                            }}>+{photos.length - 1}</span>
                          )}
                        </div>
                      )}
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        <div style={{ fontFamily: FONT_MONO, fontWeight: 600, fontSize: 16.5 }}>{fmtRM(r.cost)}</div>
