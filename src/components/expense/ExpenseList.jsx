  ...
  return (
    <div style={{ margin: '0 18px' }}>
      <div style={{ marginBottom: 12 }}>
        <span style={{ fontFamily: FONT_DISPLAY, fontSize: 17.5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Other expenses</span>
      </div>

      {expenses.length > 0 && (
        <div style={{ fontFamily: FONT_MONO, fontSize: 21.5, fontWeight: 600, marginBottom: 14 }}>
          {fmtRM(total)}
          <span style={{ fontSize: 13.5, color: COLORS.steelDim, fontFamily: FONT_BODY, marginLeft: 8 }}>total</span>
        </div>
      )}
      ...
      {expenses.length === 0 ? (
        <div style={{
          ...
          <div style={{
            ...
            <Receipt size={20} color={COLORS.steelDim} />
          </div>
          <div style={{ fontSize: 14.5 }}>Nothing logged yet — parking, tolls, road tax, insurance, anything that isn't a service.</div>
        </div>
      ) : (
              ...
              <SwipeToDelete key={e.id} onDelete={() => onDelete(e.id)}>
                <div className="csl-card" style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 13.5, color: COLORS.steel, fontFamily: FONT_MONO }}>{fmtDate(e.date)}</div>
                    {e.description && <div style={{ fontSize: 14, color: COLORS.steelDim, marginTop: 3 }}>{e.description}</div>}
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontFamily: FONT_MONO, fontWeight: 600, fontSize: 16.5 }}>{fmtRM(e.amount)}</div>
