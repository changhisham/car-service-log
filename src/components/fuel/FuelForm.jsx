...
export function FuelForm({ initial, currentOdo, onSave, onCancel }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <FormSection icon={Fuel} label="Fill-up details" tone="green" first>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
          ...
          <div>
            <div style={{ fontFamily: FONT_BODY, fontSize: 15, fontWeight: 600 }}>Full tank</div>
            <div style={{ fontFamily: FONT_BODY, fontSize: 13.5, color: COLORS.steelDim }}>Needed to calculate fuel economy accurately</div>
          </div>
        </label>
      </FormSection>
      <FormActions onCancel={onCancel} disabled={!canSave} onSave={() => onSave(f)} saveLabel={initial ? 'Save changes' : 'Add fill-up'} />
    </div>
  );
}
