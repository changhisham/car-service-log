  ...
  return (
    <div style={{ margin: '0 18px' }}>
      <div style={{ marginBottom: 12 }}>
        <span style={{ fontFamily: FONT_DISPLAY, fontSize: 17.5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Fuel</span>
      </div>
      ...
      {stats.entries.length > 0 && (
        <div className="csl-card" style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: sparkValues.length >= 2 ? 14 : 0 }}>
            <div>
              <div style={{ fontFamily: FONT_MONO, fontSize: 21.5, fontWeight: 600 }}>
                {stats.avgKmPerL ? stats.avgKmPerL.toFixed(1) : '—'}
              </div>
              <div style={{ fontSize: 13, color: COLORS.steelDim }}>avg km/L</div>
            </div>
            <div>
              <div style={{ fontFamily: FONT_MONO, fontSize: 21.5, fontWeight: 600 }}>
                {stats.avgLPer100km ? stats.avgLPer100km.toFixed(1) : '—'}
              </div>
              <div style={{ fontSize: 13, color: COLORS.steelDim }}>avg L/100km</div>
            </div>
            <div>
              <div style={{ fontFamily: FONT_MONO, fontSize: 17.5, fontWeight: 600 }}>{fmtRM(stats.totalSpent)}</div>
              <div style={{ fontSize: 13, color: COLORS.steelDim }}>total fuel spend</div>
            </div>
            <div>
              <div style={{ fontFamily: FONT_MONO, fontSize: 17.5, fontWeight: 600 }}>
                {stats.avgCostPerKm ? `RM ${stats.avgCostPerKm.toFixed(2)}` : '—'}
              </div>
              <div style={{ fontSize: 13, color: COLORS.steelDim }}>avg cost/km</div>
            </div>
          </div>
          {sparkValues.length >= 2 && (
            <div style={{ borderTop: `1px solid ${COLORS.line}`, paddingTop: 12 }}>
              <div style={{ fontSize: 13, color: COLORS.steelDim, marginBottom: 6 }}>km/L trend</div>
      ...
      {stats.entries.length === 0 ? (
        <div style={{
          ...
          <div style={{ fontSize: 14.5 }}>No fill-ups logged yet. Add one to start tracking fuel economy.</div>
        </div>
      ) : (
            ...
            <SwipeToDelete key={f.id} onDelete={() => onDelete(f.id)}>
              <div className="csl-card" style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: FONT_BODY, fontWeight: 700, fontSize: 15.5, marginBottom: 2 }}>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontFamily: FONT_MONO, fontWeight: 600, fontSize: 16.5 }}>{fmtRM(f.totalCost)}</div>
