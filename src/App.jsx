...
function Garage({ userEmail }) {
  ...
  return (
    <div className="gl-app-shell">
      ...
      {showAddRecord && active && <Modal title="Add service record" subtitle="Engine oil, brakes, tyres and more" icon={Wrench} onClose={()=>setShowAddRecord(false)}><RecordForm currentOdo={active.odometer} onSave={r=>{addRecord(r);setShowAddRecord(false)}} onCancel={()=>setShowAddRecord(false)}/></Modal>}
      {editRecord && <Modal title="Edit service record" subtitle="Engine oil, brakes, tyres and more" icon={Wrench} onClose={()=>setEditRecord(null)}><RecordForm initial={editRecord} onSave={r=>{saveEditedRecord(r);setEditRecord(null)}} onCancel={()=>setEditRecord(null)}/></Modal>}
